...
{/*
  ...
  !! THE LIVE PAGE PUBLISHES SHOPIFY'S TEMPLATE NOTES — the bracketed
  "[NOTE TO MERCHANT: …]" lines below are instructions to the store owner,
  not policy text, and one sentence introduces a list of disclosed categories
  that is not there ("denoted by *):" followed by nothing). Kept exactly as
  live, because this phase changes nothing a page says; flagged to Asim to
  have them removed (or the policy rewritten) before launch.
*/}
...
We use Shopify's ad services such as Shopify Audiences to help personalize the advertising you see on third party websites. To restrict Shopify merchants that use these ad services from using your personal information for such services, visit [https://privacy.shopify.com/en](https://privacy.shopify.com/en).
